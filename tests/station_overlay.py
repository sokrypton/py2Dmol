...
filled that set while the station fast path did not. Under Keep SSE the picture
...
keep sse", which is exactly what it looks like: the halo sits off the ribbon,
...
Keep SSE pins the secondary structure, and a residue held as helix is smoothed
...
measurement of the feature. Against a rebuild with Keep SSE still on, everything
...
window.addEventListener('load', () => {
  ...
  window.__go = async (file, keepSse) => {
    ...
    return {keepSse, took, missed, samples: fx.length,
...
runs = {}
for tag, keep in (("Keep SSE", True), ("plain", False)):
...
# 🔴 AND THE FAST PATH HAS TO HAVE RUN, or the Keep SSE arm is just a rebuild
# wearing the mode's name and would pass with the bug fully present.
if runs["Keep SSE"]["took"] <= 0:
    bad.append("the fast path never ran under Keep SSE, so nothing here"
