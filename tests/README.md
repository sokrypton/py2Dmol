...
a probe that is added and never run shows up here as `tool` rather than
disappearing into the directory. `tool` is right for the benches, the fitting
scripts and the by-hand demonstrations; on anything else it is a question.

<!-- INDEX: generated by tests/index.py, do not edit by hand -->
...
| `hover_echo.py` | ui | A rebuilt sequence strip forgets the hover; a copy does not echo. |
| `interaction.js` | node | Gesture-degrade tests for core/mol.js: the outline drop during a gesture, |
...
| `richardson_tint.py` | ui | The pale inner face belongs to the helix, and stops where the helix does. |
...
in `docs/PAINT_ORDER.md` at the repo root. The work itself is on the `paint-order/*` branches.
