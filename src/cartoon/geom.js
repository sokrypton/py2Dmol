...
//   "tube" drawing. core/mol.js delegates here from _renderToContext()
...
//   updates, PAE/seq/scatter sync) stays in core/mol.js and keeps working.
...
// ------------------------------------------------------------------------
// Thresholds for the C-alpha-only secondary-structure assignment.
// ------------------------------------------------------------------------
//
// 🔴 THE ASSIGNMENT ITSELF IS NOT IN THIS FILE ANY MORE - see
// tests/ss_legacy.js. It is the superseded TMalign make_sec pipeline, kept
// only as the baseline tests/ss_bench.js scores the shipped assignment
// against, and nothing the app can reach calls it. It shipped in every
// bundle regardless, so every reader downloaded and parsed it to run a
// benchmark they were not running.
//
// These parameters stay because the LIVE path reads two of them:
// `SS.chainMax` sizes the ribbon's loop, and `SS.minStrand` drops a short
// strand run in assignSecondary. The rest are the baseline's, and
// tests/ss_tune.js sweeps them through py2dmolCartoon.SS_PARAMS.
...
// --- HAND-DRAWN BUILD-UP (see animateDrawing in core/mol.js) ----------
...
// THE SAME NUMBERS PYMOL USES, taken from its ElementTable (pymol-open-source,
// layer2/AtomInfo.cpp). Bondi's van der Waals radii where Bondi has them, and
// 1.80 for everything else - which is most of the metals, and is why this is
// not simply "the vdW radii": the published sets disagree wildly above argon
// and Alvarez gives calcium 2.31 where PyMOL gives 1.80.
//
// Matching a reference people already have on screen beats being defensibly
// right on our own. Both complaints about the calcium were the distance from
// this table: 2.31 was visibly too big, and an ionic 1.00 visibly too small.
const VDW_A = {
    H: 1.20, LI: 1.82, C: 1.70, N: 1.55, O: 1.52, F: 1.47, NA: 2.27,
    MG: 1.73, AL: 2.00, SI: 2.10, P: 1.80, S: 1.80, CL: 1.75, K: 2.75,
    CA: 1.80, MN: 1.73, FE: 1.80, CO: 1.80, NI: 1.63, CU: 1.40, ZN: 1.39,
    SE: 1.90, BR: 1.85, RB: 1.80, SR: 1.80, MO: 1.80, AG: 1.72, CD: 1.58,
    I: 1.98, CS: 1.80, BA: 1.80, W: 1.80, PT: 1.75, AU: 1.66, HG: 1.55,
    PB: 2.02,
};
// PyMOL's own default for an element its table does not reach, rather than
// carbon's - an unknown here is far more likely to be a heavy atom than a
// light one.
const VDW_DEFAULT = 1.80;
// AT THE FULL RADIUS, not a fraction of it. A lone atom is the one thing in
// this drawing that IS an atom - everything else stands for a residue - so it
// is drawn the size an atom is. Zinc comes out smaller than a carbon, which is
// not what people expect of a metal and is what the table says.
const ION_VDW_FRAC = 1.0;
/**
 * How big a lone atom of this element is drawn, in Angstrom.
 *
 * EXPORTED, because two files need the same number: this one draws the ball,
...
// The SAME number core/mol.js calls CONTACT_WIDTH_A - a contact that
...
// core/mol.js paints the canvas with it when not transparent; the depth
...
// (core/mol.js, gapFillerColor), so outlines follow the colour scheme.
...
// element.
const RIBBON_INK_MUL = 0.7;   // matches core/mol.js gapFillerColor
...
/**
 * The rungs of a duplex: one plate per base, cut where it meets the rail.
 *
 * FIVE HUNDRED AND EIGHTY-EIGHT LINES OUT OF render(), which was 6,890 and is
 * the largest single thing in this repo. Base plates are a self-contained
 * question - given the nucleic frames and the smoothed rails, which residues
 * show a plate and what shape is it - and they touch nothing after them:
 * everything they produce goes into `prims` and `renderer._naPick`.
 *
 * It takes a state object rather than twenty-two arguments, the same shape
 * core/mol.js hands _paintTubeSegments. The other five names the range reads -
 * LIGHT, NA_AXIS_WIN, NA_JOINT_CLEAR, NA_PLATE_SEG and subFloor - are declared
 * at this file's own scope and need no passing; free_vars lists them because
 * they are outside the RANGE, not because they are out of reach.
 *
 * @param {object} S - what render() worked out before it got here
 */
function buildBasePlates(S) {
...
// own set in its own numbering (see mergedObjectSet in core/mol.js).
...
/**
 * Bonds joined into runs, and the joints where runs meet.
 *
 * A side chain is drawn as few long boxes rather than many short ones: a run
 * of collinear bonds with the same width and colour becomes one primitive,
 * and where runs meet, a mitre or a collar closes the corner.
 *
 * A THOUSAND AND SEVENTY-EIGHT LINES OUT OF render(). It was already a bare
 * `{ ... }` block - the author scoped it by hand so its locals could not leak
 * - which is the same judgement this makes one level further out, and is why
 * nothing needed untangling: free_vars found twenty names crossing the
 * boundary and three of those are this file's own constants.
 *
 * @param {object} S - what render() built before the sticks were joined
 */
...
function render(renderer, ctx, displayWidth, displayHeight, colors) {
    ...
    // screen pixels into Angstroms (see the pan handler in core/mol.js)
    ...
    const genericSegs = [];
    // HEAD-TO-TAIL CLOSURE (cyclic peptides). core/mol.js emits a bond
    ...
    // (animateDrawing, core/mol.js) it carries how far each MEDIUM has
    ...
    // read by core/mol.js, which re-renders once the gesture settles -
    ...
    // core/mol.js, which the draw calls once this returns.
    //
    ...
    // absolute stroke width here, unlike core/mol.js's
...
function drawRun(runIdx, ctx) {
    const {
        ...
        for (let i = lo; i <= lastStep; i++) {
            ...
            // core/mol.js gives every segment the colour of ONE residue -
...
function drawSticks(ctx) {
    ...
    // here (see _materialiseSidechains in core/mol.js), so the generic
    ...
    // ...and the sticks joined into runs, with their joints closed.
    mergeBondRuns({
...
// THE PAINTER IS A SEPARATE FILE, and may not be in this build: the
// WebGL target ships the geometry and cartoon/paintgl.js and no 2D
// painter at all. Say so rather than returning quietly, which would be a
// blank canvas with nothing in the console.
function paintPrims(S) {
    const paint = window.py2dmolCartoonPaint;
    if (!paint) {
        if (!render._warnedNoPainter) {
            render._warnedNoPainter = true;
            console.error("py2dmol: no 2D cartoon painter in this build"
                + " - load cartoon/paint2d.js, or use the WebGL path");
        }
        return undefined;
    }
    return paint(S);
...
// Per-style preset values, in ONE place. core/mol.js applies these on a
...
const LOOK_DEFAULTS = {
    ...
    // in core/mol.js still lets a real drag survive a switch, but with the
...
// and tests/ss_bench.js: scoring a reimplementation lets the two drift apart
// silently, so the benchmarks call the same functions the renderer calls.
//
// 🔴 makeSec, smoothSec AND extendSec ARE NOT HERE ANY MORE. They were the
// superseded C-alpha-only pipeline, exported for no reason but that the
// benchmarks compare against them - nothing the app can reach called them, and
// they shipped in every bundle anyway. They live in tests/ss_legacy.js now,
// which loads into the same context and puts them back on this object; the app
// neither downloads nor parses them. SS_PARAMS stays because the live path
// reads two of its fields.

window.py2dmolCartoon = { render, SS_PARAMS: SS,
 ...
 * same total length.
 */
// THE SAME STATEMENT THE MESH KEYS ASK FOR (core/mol.js:_coordsKey):
...
// window", which was simply untrue - core/mol.js declares at global scope, so
// its functions are as reachable as getAllValidColorModes is from three other
// files. The real reason is thirteen test harnesses that load this file ALONE,
// against a mock canvas, with no renderer anywhere: calling into core would
// make every one of them throw on load. A subsystem that can be loaded by
// itself cannot reach upward, so it writes the table it shares.
...
// WHAT THE PAINTERS SHARE. cartoon/paint2d.js reads these 36 names - the
// shading and sketching vocabulary - and nothing else from this file. The
// list is computed, not curated: every name declared at this scope that the
// painter mentions.
//
// AT THE END OF THE IIFE ON PURPOSE. It was where paintPrims used to sit,
// which is before SHEET_EDGE_RGB is declared - so that one could not be in
// it, and the painter died on a sheet edge in the browser while every node
// fixture passed.
//
// Kept off py2dmolCartoon deliberately: that object is the public API, and
// this is a seam between two halves of one renderer.
window.py2dmolCartoonShared = {
