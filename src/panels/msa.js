...
(function () {
    ...
    /**
     * The records in a FASTA-shaped file: `>header` then sequence lines.
     *
     * ONE READER, TWO FORMATS. parseA3M and parseFasta each carried their own
     * copy of this loop - twenty-five identical lines whose ONLY difference was
     * what they did to the accumulated sequence. A3M strips the lowercase
     * insertion columns; plain FASTA keeps the gaps and only upper-cases. That
     * is the `clean` argument, and it is the whole of the difference.
     *
     * The two copies had already begun to drift in their comments, which is how
     * copies go before they drift in their code.
     */
    function readFastaRecords(fileContent, clean) {
    ...
    // A3M: lowercase columns are insertions relative to the query, and are
    // dropped so every row lines up with it.
    const A3M_CLEAN = (seq) => seq.replace(/[a-z]/g, '').toUpperCase();
    // FASTA: the alignment is already rectangular; keep the gaps as they are.
    const FASTA_CLEAN = (seq) => seq.toUpperCase();

    function parseA3M(fileContent) {
        const sequences = readFastaRecords(fileContent, A3M_CLEAN);
    ...
    function parseFasta(fileContent) {
        const sequences = readFastaRecords(fileContent, FASTA_CLEAN);
    ...
    window.MSA = {
        ...
        // MSA utility functions (moved from the web app and core/mol.js)
