...
'''
with open("pseudo_3D_viewer.html","w") as handle:
...
# --- Updated ProteinViewerGUI Class ---

class ProteinViewerGUI:
    def __init__(self, size=(500,500), color="plddt", html_template_path="pseudo_3D_viewer.html"):
    ...
    def clear(self):
        ...
        if self._initial_data_loaded:
            try:
                # This new JS function will create a new trajectory in the dropdown
                output.eval_js(f"window.handlePythonNewTrajectory('{trajectory_name}');")
            except Exception as e:
                # This might fail if the cell output was cleared, which is fine
                pass
    ...
    def display(self, initial_coords, initial_plddts=None, initial_chains=None, initial_atom_types=None):
        ...
        if not os.path.exists(self.html_template_path):
            print(f"Error: HTML template file not found at '{self.html_template_path}'")
