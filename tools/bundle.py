...
MODULES = [
    ...
    Mod('slots',      'src/parts/slots.js',     ['web']),
...
# 🔴 THE REBUILD LIGHT, AND IT IS ONLY EVER ON dev.html.
#
# A cartoon frame is either REBUILT - the capture, facesOf, buildMeshPart, the
# whole mesh - or answered from the station table, which uploads new positions
# into a mesh that already exists. The difference is the whole point of Keep
# SSE and it is invisible: both draw the same picture, one of them at several
# times the cost. So the page says which, and a trajectory that is quietly
# rebuilding every frame stops being something you have to instrument to
# notice.
#
# INJECTED HERE RATHER THAN WRITTEN INTO index.html, so it cannot ship. dev.html
# is generated from index.html, and this is the one thing the two pages differ
# by beyond the script tags.
#
# 🔴 IT WATCHES window.__faceBuilds, WHICH IS THE GPU CARTOON'S COUNTER. A tube
# frame or a 2D fallback increments nothing and lights nothing - the light means
# "the cartoon mesh was rebuilt", not "something happened". Anything else would
...
def render_dev():
    ...
    out = src.replace('    ' + tag + '\n', DEV_BANNER + loose, 1)
    if out.count('</body>') != 1:
        sys.exit(f'{SITE_HTML} must carry exactly one </body>')
    return out.replace('</body>', DEV_REBUILD_LIGHT + '</body>', 1)
